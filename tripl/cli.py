...
def add_base_arguments(parser):
    ...
    parser.add_argument('-m', '--map-command',
        help="""map each input through this command, al a `map-command input`; command should write as json to
        stdout a list of fact dictionaries, or a single, nested EAV index dictionary, as json, to stdout.
        """)
...
def get_args():
    ...
    join_parser = subparsers.add_parser('join',
        help="""Join data from some number of files or directories, subject to --map-command output interpretation as
        tripl data where applied, and spit out to -o tripl.json file as an eav index mapping.""")
    add_base_arguments(join_parser) #lint

    # Now for pull
    pull_parser = subparsers.add_parser('pull',
        help="""Pull document/json structure out of ingested tripl data, given pull expression, and save to -o.""")
    ...
    pull_parser.add_argument('-N', '--drop-namespaces', help='MOCK! output results with namespaces removed from keywords')
